...
const styles = {
  ...
  sidebar: { background: "#ffffff", borderRadius: "20px", padding: "20px", boxShadow: "0 4px 20px rgba(0,0,0,0.06)", height: "fit-content" },
  ...
  levelButton: (active) => ({ padding: "8px 12px", borderRadius: "999px", border: "none", cursor: "pointer", margin: "4px", background: active ? "#0f172a" : "#e2e8f0", color: active ? "#ffffff" : "#0f172a" }),
  cardGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "16px" },
  card: { background: "#ffffff", borderRadius: "20px", padding: "20px", boxShadow: "0 4px 20px rgba(0,0,0,0.06)" },
  badge: { display: "inline-block", padding: "6px 10px", borderRadius: "999px", background: "#e2e8f0", marginRight: "8px", marginBottom: "8px", fontSize: "12px" },
...
export default function App() {
  ...
  const filteredStudents = useMemo(() => studentsSeed.filter((student) => {
  ...
  }).map((student) => ({ ...student, shared: student.interests.filter((interest) => currentInterestList.includes(interest)) })), [selectedLevel, search, currentInterestList]);
  ...
  const handleAuthSubmit = () => {
    ...
    const userProfile = { name: authForm.name.trim() || profile.name || "Student", level: profile.level, interests: profile.interests, goal: profile.goal, email: authForm.email.trim() };
  ...
  const groupIdForStudent = (student) => student.level === "Year 7" ? 1 : student.level === "Year 9" ? 2 : 3;
  ...
  const addGroupPost = () => {
    ...
    setGroups((prev) => prev.map((group) => group.id === activeGroupId ? { ...group, posts: [`${profile.name}: ${groupPostText}`, ...group.posts] } : group));
    setGroupPostText("");
  };

  const addPost = () => {
    if (!newPost.trim()) return;
    const post = { id: posts.length + 1, author: isLoggedIn ? profile.name : "Guest", level: selectedLevel === "All" ? profile.level : selectedLevel, text: newPost, tags: currentInterestList.length ? [currentInterestList[0]] : ["Study"] };
    setPosts([post, ...posts]);
    setNewPost("");
  };
        ...
        <div style={styles.hero}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: "16px", alignItems: "center", flexWrap: "wrap" }}>
            ...
            <div style={{ minWidth: "220px", textAlign: "right" }}>
              <div style={{ fontWeight: "700", marginBottom: "8px" }}>{isLoggedIn ? `Welcome, ${profile.name}` : "Student login"}</div>
              <div style={{ color: "#475569", marginBottom: "10px" }}>{isLoggedIn ? "You can now join groups and post updates." : "Log in to join groups and make them active."}</div>
        ...
        <div style={styles.layout}>
          <div>
            <div style={styles.sidebar}>
              ...
              <div>{levels.map((level) => <button key={level} style={styles.levelButton(selectedLevel === level)} onClick={() => setSelectedLevel(level)}>{level}</button>)}</div>
            </div>
          ...
          <div>
            ...
            {activeTab === "students" && <div style={styles.cardGrid}>{filteredStudents.map((student) => <div key={student.id} style={styles.card}><h3>{student.name}</h3><p><strong>{student.level}</strong></p><p>{student.goal}</p><p>{student.bio}</p><p><strong>Available:</strong> {student.availability}</p><div style={{ marginTop: "10px" }}>{student.interests.map((interest) => <span key={interest} style={styles.badge}>{interest}</span>)}</div>{student.shared.length > 0 && <p><strong>Shared interests:</strong> {student.shared.join(", ")}</p>}<button style={styles.button} onClick={() => joinGroup(groupIdForStudent(student))}>Connect</button></div>)}</div>}

            {activeTab === "groups" && <div><div style={styles.cardGrid}>{filteredGroups.map((group) => <div key={group.id} style={styles.card}><h3>{group.name}</h3><p><strong>{group.level}</strong></p><p><strong>Topic:</strong> {group.topic}</p><p>{group.description}</p><p><strong>Members:</strong> {group.members}</p><p><strong>Next meeting:</strong> {group.meeting}</p><button style={styles.secondaryButton} onClick={() => joinGroup(group.id)}>Join group</button><button style={{ ...styles.button, marginLeft: "8px" }} onClick={() => setActiveGroupId(group.id)}>Open group</button></div>)}</div><div style={{ ...styles.card, marginTop: "20px" }}><h3>{activeGroup.name}</h3><p><strong>Active room:</strong> {activeGroup.topic} · {activeGroup.meeting}</p><p>{activeGroup.description}</p><textarea style={styles.textarea} value={groupPostText} onChange={(e) => setGroupPostText(e.target.value)} placeholder={isLoggedIn ? "Write an update for the group" : "Log in to post in the group"} disabled={!isLoggedIn} /><button style={styles.button} onClick={addGroupPost} disabled={!isLoggedIn}>Post to group</button><div style={{ marginTop: "16px" }}>{activeGroup.posts.map((post, index) => <div key={index} style={{ padding: "12px 0", borderBottom: "1px solid #e2e8f0" }}>{post}</div>)}</div></div></div>}

            {activeTab === "feed" && <div><div style={styles.card}><h3>Share a math post</h3><textarea style={styles.textarea} value={newPost} onChange={(e) => setNewPost(e.target.value)} placeholder="Ask a question or invite students to study" /><button style={styles.button} onClick={addPost}>Post</button></div><div style={{ marginTop: "16px" }}>{filteredPosts.map((post) => <div key={post.id} style={{ ...styles.card, marginBottom: "16px" }}><h3>{post.author}</h3><p><strong>{post.level}</strong></p><p>{post.text}</p><div>{post.tags.map((tag) => <span key={tag} style={styles.badge}>#{tag}</span>)}</div></div>)}</div></div>}

            {activeTab === "practice" && <div><div style={styles.card}><h3>Free Math Practice</h3><p style={{ color: "#475569" }}>Choose your year level, pick a topic, and practise with guided answers and explanations.</p><div style={{ marginTop: "12px", marginBottom: "16px" }}>{practiceLevels.map((level) => <button key={level} style={styles.levelButton(practiceLevel === level)} onClick={() => changePracticeLevel(level)}>{level}</button>)}</div><div style={styles.cardGrid}>{activePracticeTopics.map((topic, index) => <div key={topic.topic} style={styles.card}><h3>{topic.topic}</h3><p>{topic.sets.length} practice set{topic.sets.length > 1 ? "s" : ""}</p><button style={styles.button} onClick={() => { setSelectedTopicIndex(index); setSelectedSetIndex(0); setVisibleAnswers({}); setCheckedAnswers({}); }}>Open topic</button></div>)}</div></div>
              {activePracticeTopic && activePracticeSet && <div id="practice-section" style={{ ...styles.card, marginTop: "20px" }}><h3>{practiceLevel} · {activePracticeTopic.topic}</h3><p><strong>{activePracticeSet.title}</strong></p><div style={{ marginTop: "16px", padding: "16px", borderRadius: "16px", background: "#f8fafc", border: "1px solid #e2e8f0" }}><div style={{ display: "flex", justifyContent: "space-between", gap: "12px", flexWrap: "wrap" }}><div><div style={{ fontWeight: "700" }}>Your progress</div><div style={{ color: "#475569" }}>{completedInSet} of {totalQuestionsInSet} questions completed</div></div><div style={{ fontWeight: "700" }}>{progressPercent}%</div></div><div style={styles.progressBarOuter}><div style={{ width: `${progressPercent}%`, height: "100%", background: "#0f172a" }} /></div></div><div style={{ marginTop: "10px", marginBottom: "16px" }}>{activePracticeTopic.sets.map((setItem, index) => <button key={setItem.title} style={styles.levelButton(selectedSetIndex === index)} onClick={() => { setSelectedSetIndex(index); setVisibleAnswers({}); setCheckedAnswers({}); }}>{setItem.title}</button>)}</div>
              {activePracticeSet.questions.map((item) => <div key={item.id} style={styles.questionCard}><div style={{ display: "flex", justifyContent: "space-between", gap: "12px", alignItems: "center", flexWrap: "wrap" }}><div style={{ fontWeight: "700", marginBottom: "8px" }}>Question</div><label style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "14px" }}><input type="checkbox" checked={!!completedQuestions[item.id]} onChange={() => toggleCompleted(item.id)} />Mark complete</label></div><div style={{ marginBottom: "10px" }}>{item.question}</div><input style={styles.input} value={studentAnswers[item.id] || ""} onChange={(e) => updateStudentAnswer(item.id, e.target.value)} placeholder="Type your answer here" /><div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}><button style={styles.button} onClick={() => checkAnswer(item)}>Check answer</button><button style={styles.secondaryButton} onClick={() => toggleAnswer(item.id)}>{visibleAnswers[item.id] ? "Hide answer" : "Show answer"}</button></div>{checkedAnswers[item.id]?.attempted && <div style={{ marginTop: "12px", padding: "10px 12px", borderRadius: "12px", background: checkedAnswers[item.id].isCorrect ? "#dcfce7" : "#fee2e2" }}>{checkedAnswers[item.id].isCorrect ? "Correct — well done!" : "Not quite yet — check the explanation or try again."}</div>}{visibleAnswers[item.id] && <div style={{ marginTop: "12px" }}><div><strong>Answer:</strong> {item.answer}</div><div style={{ color: "#475569", marginTop: "6px" }}><strong>How to solve:</strong> {item.explanation}</div><div style={{ color: "#475569", marginTop: "6px" }}><strong>Common mistake:</strong> {item.commonMistake}</div></div>}</div>)}
              <div style={{ marginTop: "20px", padding: "16px", borderRadius: "16px", background: "#e0f2fe" }}><div style={{ fontWeight: "700", marginBottom: "8px" }}>Want a personalised study plan?</div><div style={{ color: "#0f172a", marginBottom: "8px" }}>Join the student portal to get guided practice, extra materials, support tailored to your level, answer checking, and a clearer picture of your progress.</div><div style={{ color: "#0f172a" }}>You will also get step-by-step feedback and a progress chart to help you improve faster.</div><button style={styles.button} onClick={() => setActiveTab("login")}>Get personalised help</button></div></div>}</div>}

            {activeTab === "chat" && <div style={styles.chatBox}><h3>Private chat demo</h3><div style={{ marginBottom: "16px", marginTop: "16px" }}>{messages.map((message) => <div key={message.id} style={styles.message(message.from === "You" || message.from === profile.name)}><div style={{ fontSize: "12px", marginBottom: "4px", opacity: 0.8 }}>{message.from}</div><div>{message.text}</div></div>)}</div><input style={styles.input} value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder={isLoggedIn ? "Type a message" : "Type a guest message"} /><button style={styles.button} onClick={sendMessage}>Send</button></div>}

            {activeTab === "login" && <div style={styles.card}><h3>{loginMode === "login" ? "Student login" : "Create account"}</h3><p style={{ color: "#475569" }}>This version uses simple local login so you can test the app before adding Firebase.</p>{authMessage && <div style={{ marginBottom: "12px", padding: "10px 12px", borderRadius: "12px", background: "#e0f2fe", color: "#0f172a" }}>{authMessage}</div>}{loginMode === "signup" && <input style={styles.input} value={authForm.name} onChange={(e) => setAuthForm({ ...authForm, name: e.target.value })} placeholder="Your name" />}<input style={styles.input} value={authForm.email} onChange={(e) => setAuthForm({ ...authForm, email: e.target.value })} placeholder="Email" /><input style={styles.input} type="password" value={authForm.password} onChange={(e) => setAuthForm({ ...authForm, password: e.target.value })} placeholder="Password" /><button style={styles.button} onClick={handleAuthSubmit}>{loginMode === "login" ? "Log in" : "Create account"}</button><button style={{ ...styles.secondaryButton, marginLeft: "8px" }} onClick={() => { setLoginMode(loginMode === "login" ? "signup" : "login"); setAuthMessage(""); }}>Switch to {loginMode === "login" ? "sign up" : "login"}</button></div>}
          </div>
        </div>
      </div>
    </div>
